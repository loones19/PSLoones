...
{block name='content'}
	...
	<div id="blog-listing" class="blogs-container box">
		{if isset($filter.type)}
			{if $filter.type=='tag'}
				<h1>{l s='Filter Blogs By Tag' d='Psblog'} : <span>{$filter.tag|escape:'html':'UTF-8'}</span></h1>
			{elseif $filter.type=='author'}
				<h1>{l s='Filter Blogs By Blogger' d='Psblog'} : <span>{$filter.employee->firstname|escape:'html':'UTF-8'} {$filter.employee->lastname|escape:'html':'UTF-8'}</span></h1>
			{/if}
		{else}
			<h1 class="blog-heading h1">{l s='Latest Blogs' d='Psblog'}</h1>
		{/if}
		...
		<div class="inner">
			...
			{else}
				<div class="alert alert-warning">{l s='Sorry, We are update data, please come back later!!!!' d='Psblog'}</div>
			{/if}    
