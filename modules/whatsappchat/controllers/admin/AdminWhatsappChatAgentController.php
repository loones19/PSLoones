...
class AdminWhatsappChatAgentController extends ModuleAdminController
{
    ...
    public function processAdd()
    {
        ...
        if (substr(Tools::getValue('image'), 0, 1) == '_') {
            $_POST['image'] = substr(Tools::getValue('image'), 1);
        }
        ...
        if (version_compare(_PS_VERSION_, '1.7', '>=')) {
            if (Tools::getValue('image') && Tools::getValue('image') != '') {
                ...
                if (Tools::getValue('image') == ' ') {
                    ...
                    if (substr($processAdd->image, 0, 1) == '_') {
                        $processAdd->image = substr($processAdd->image, 1);
                    }
