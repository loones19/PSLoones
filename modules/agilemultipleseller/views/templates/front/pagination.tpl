...
<div class="row">
    ...
    <div class="col-md-8 col-sm-6 col-xs-12">
        ...
        {/if}
        {if $nb_products > 1}
        	{l s='Showing %1$d - %2$d of %3$d items' sprintf=[$productShowingStart, $productShowing, $nb_products] mod='agilemultipleseller'}
		{else}
        	{l s='Showing %1$d - %2$d of 1 item' sprintf=[$productShowingStart, $productShowing] mod='agilemultipleseller'}
       	{/if}
    </div>
			...
			<form class="showall" action="{if !is_array($requestNb)}{$requestNb}{else}{$requestNb.requestUrl}{/if}" method="get">
				<div>
	                ...
	                <button type="submit" class="btn btn-default button exclusive-medium">
	                	<span>{l s='Show all' mod='agilemultipleseller'}</span>
	                </button>
		...
		{if $start!=$stop}
			<div class="pagination">
				{if $p != 1}
					...
					<span id="pagination_previous{if isset($paginationId)}_{$paginationId}{/if}" class="pagination_previous">
						<a{$no_follow_text} href="{$link->goPage($requestPage, $p_previous)}" rel="prev">
							<i class="icon-chevron-left"></i> <b>{l s='Prev' mod='agilemultipleseller'}</b>
						</a>
					</span>
				{else}
					<span id="pagination_previous{if isset($paginationId)}_{$paginationId}{/if}" class="disabled pagination_previous">
						<span>
							<i class="icon-chevron-left"></i> <b>{l s='Prev' mod='agilemultipleseller'}</b>
						</span>
					</span>
				...
				{if $pages_nb > 1 AND $p != $pages_nb}
					...
					<span id="pagination_next{if isset($paginationId)}_{$paginationId}{/if}" class="pagination_next">
						<a{$no_follow_text} href="{$link->goPage($requestPage, $p_next)}" rel="next">
							<b>{l s='Next' mod='agilemultipleseller'}</b> <i class="icon-chevron-right"></i>
						</a>
					</span>
				{else}
					<span id="pagination_next{if isset($paginationId)}_{$paginationId}{/if}" class="disabled pagination_next">
						<span>
							<b>{l s='Next' mod='agilemultipleseller'}</b> <i class="icon-chevron-right"></i>
						</span>
					</span>
