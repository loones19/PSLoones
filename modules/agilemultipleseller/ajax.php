...
if (Tools::getValue('submitPublishProduct'))
{
	if (Tools::getIsset('id_product'))
	{
		...
		$token = Tools::getAdminToken('AdminProducts'.(int)($id_tab_catalog).(int)$context->cookie->id_employee);
		$bo_product_url = dirname($_SERVER['PHP_SELF']).'/index.php?tab=AdminProducts&id_product='.$id_product.'&updateproduct&token='.$token;

		if (Tools::getValue('redirect'))
			die($bo_product_url);

		$profileAccess = Profile::getProfileAccess($context->cookie->profile, $id_tab_catalog);
...
if (Tools::getValue('submitPublishCMS'))
{
	if (Tools::getIsset('id_cms'))
	{
		...
		$token = Tools::getAdminToken('AdminCmsContent'.(int)($id_tab_cms).(int)$context->cookie->id_employee);
		$bo_cms_url = dirname($_SERVER['PHP_SELF']).'/index.php?tab=AdminCmsContent&id_cms='.(int)$id_cms.'&updatecms&token='.$token;

		if (Tools::getValue('redirect'))
			die($bo_cms_url);

		$profileAccess = Profile::getProfileAccess($context->cookie->profile, $id_tab_cms);
		if($profileAccess['edit'])
		{
