...
//view type
    ...
    /* PrestaShop */
	$sellerfolder = ""; 
	if(Module::isInstalled('agilemultipleseller'))$sellerfolder = "sellers/" . $sellerinfo->id_seller . "/";
	if(isset($src_thumb) && $src_thumb){
		if (($src_thumb = preg_replace('#('.addslashes($current_path).')#ism', Tools::safeOutput(Context::getContext()->shop->physical_uri.'img/cms/' . $sellerfolder), $src_thumb)) == $src_thumb){
			$src_thumb = preg_replace('#('.addslashes($thumbs_base_path).')#ism', Tools::safeOutput(Context::getContext()->shop->physical_uri.'img/tmp/cms/' . $sellerfolder), $src_thumb);
		}
	}
	if(isset($mini_src) && $mini_src){
		if (($mini_src = preg_replace('#('.addslashes($current_path).')#ism', Tools::safeOutput(Context::getContext()->shop->physical_uri.'img/cms/' . $sellerfolder), $mini_src)) == $mini_src){
			$mini_src = preg_replace('#('.addslashes($thumbs_base_path).')#ism', Tools::safeOutput(Context::getContext()->shop->physical_uri.'img/tmp/cms/' . $sellerfolder), $mini_src);
		}
	}
	/* END PrestaShop */
