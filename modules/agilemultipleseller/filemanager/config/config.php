...
if(!Module::isInstalled('agilemultipleseller'))
	die(Tools::displayError('Access Permission Denied'));

require_once(_PS_ROOT_DIR_ . '/modules/agilemultipleseller/SellerInfo.php');

$sellerinfo = new SellerInfo(SellerInfo::getIdByCustomerId($_SESSION['id_customer']));

$seller = new Employee($sellerinfo->id_seller);
$seller_exists = Validate::isLoadedObject($seller);
$isSeller =  ($seller_exists AND $seller->active && $seller->id_profile == (int)Configuration::get('AGILE_MS_PROFILE_ID'));

if (!$seller_exists || !$isSeller) {
	die(Tools::displayError('Access Permission Denied'));
}

if($sellerinfo->id_seller > 0)
{
	$current_path =  _PS_ROOT_DIR_.'/img/cms/sellers';
	if(!file_exists($current_path))mkdir($current_path);
	$current_path = $current_path . '/'. $sellerinfo->id_seller;
	if(!file_exists($current_path))mkdir($current_path);
	$current_path =  $current_path . '/';

	$upload_dir = Context::getContext()->shop->getBaseURI().'img/cms/sellers/' . $sellerinfo->id_seller . '/';

	$thumbs_base_path =  _PS_ROOT_DIR_.'/img/tmp/cms'; 
	if(!file_exists($thumbs_base_path))mkdir($thumbs_base_path);
	$thumbs_base_path .= '/sellers'; 
	if(!file_exists($thumbs_base_path))mkdir($thumbs_base_path);

	$thumbs_base_path = $thumbs_base_path . '/'. $sellerinfo->id_seller;
	if(!file_exists($thumbs_base_path))mkdir($thumbs_base_path);
