...
require(dirname(__FILE__).'/tm_verticalmenutoplinks.class.php');

class Tm_VerticalMenu extends Module implements WidgetInterface
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('TM - Vertical Menu');
    ...
    public function install($delete_params = true)
    {
        ...
        if ($delete_params) {
            if (!$this->installDb() || !Configuration::updateGlobalValue('MOD_TMVERTICALMENU_ITEMS', 'CAT3')) {
                return false;
            }
        }
    ...
    public function installDb()
    {
		...
		CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'tmverticalmenu` (
			`id_tmverticalmenu` INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
			`id_shop` INT(11) UNSIGNED NOT NULL,
			`new_window` TINYINT( 1 ) NOT NULL,
			INDEX (`id_shop`)
		) ENGINE = '._MYSQL_ENGINE_.' CHARACTER SET utf8 COLLATE utf8_general_ci;') &&
            Db::getInstance()->execute('
			 CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'tmverticalmenu_lang` (
			`id_tmverticalmenu` INT(11) UNSIGNED NOT NULL,
			`id_lang` INT(11) UNSIGNED NOT NULL,
			`id_shop` INT(11) UNSIGNED NOT NULL,
			`label` VARCHAR( 128 ) NOT NULL ,
			`link` VARCHAR( 128 ) NOT NULL ,
			INDEX ( `id_tmverticalmenu` , `id_lang`, `id_shop`)
		) ENGINE = '._MYSQL_ENGINE_.' CHARACTER SET utf8 COLLATE utf8_general_ci;'));
    }
    ...
    public function uninstall($delete_params = true)
    {
        ...
        if ($delete_params) {
            if (!$this->uninstallDB() || !Configuration::deleteByName('MOD_TMVERTICALMENU_ITEMS') || !Configuration::deleteByName('MOD_TMVERTICALMENU_COLS')) {
                return false;
            }
        }
    ...
    {
        Db::getInstance()->execute('DROP TABLE IF EXISTS`'._DB_PREFIX_.'tmverticalmenu`');
        Db::getInstance()->execute('DROP TABLE IF EXISTS`'._DB_PREFIX_.'tmverticalmenu_lang`');
        return true;
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitTmverticalmenu')) {
            ...
            foreach ($shops as $shop_id) {
                ...
                if (count($shops) == 1) {
                    if (is_array($items) && count($items)) {
                        $updated = Configuration::updateValue('MOD_TMVERTICALMENU_ITEMS', (string)implode(',', $items), false, (int)$shop_group_id, (int)$shop_id);
                    } else {
                        $updated = Configuration::updateValue('MOD_TMVERTICALMENU_ITEMS', '', false, (int)$shop_group_id, (int)$shop_id);
                    }
                }
				
				$number_cols = Tools::getValue('number_cols_menu');
				$updated &= Configuration::updateValue('MOD_TMVERTICALMENU_COLS', (string)$number_cols, false, (int)$shop_group_id, (int)$shop_id);
            ...
            if (Tools::isSubmit('submitTmverticalmenuLinks')) {
                ...
                if ($count_links_label || $count_label) {
                    if (!$count_links_label) {
                        ...
                        foreach ($shops as $shop_id) {
                            $added = Tm_VerticalMenuTopLinks::add($links_label, $labels,  Tools::getValue('new_window', 0), (int)$shop_id);
                ...
                $update_cache = true;
            } elseif (Tools::isSubmit('deletetmverticalmenu')) {
                $errors_delete_link = array();
                $id_tmverticalmenu = Tools::getValue('id_tmverticalmenu', 0);
                $shops = Shop::getContextListShopID();

                foreach ($shops as $shop_id) {
                    $deleted = Tm_VerticalMenuTopLinks::remove($id_tmverticalmenu, (int)$shop_id);
                    Configuration::updateValue('MOD_TMVERTICALMENU_ITEMS', str_replace(array('LNK'.$id_tmverticalmenu.',', 'LNK'.$id_tmverticalmenu), '', Configuration::get('MOD_TMVERTICALMENU_ITEMS')));
                ...
                $update_cache = true;
            } elseif (Tools::isSubmit('updatetmverticalmenu')) {
                $id_tmverticalmenu = (int)Tools::getValue('id_tmverticalmenu', 0);
                ...
                if (Tools::isSubmit('updatelink')) {
                    ...
                    Tm_VerticalMenuTopLinks::update($link, $label, $new_window, (int)$id_shop, (int)$id_tmverticalmenu, (int)$id_tmverticalmenu);
        ...
        foreach ($shops as $shop_id) {
            $links = array_merge($links, Tm_VerticalMenuTopLinks::gets((int)$id_lang, null, (int)$shop_id));
        }
        ...
        if (is_array($items) && count($items)) {
            ...
            if (count($shops) > 1) {
                foreach ($shops as $key => $shop_id) {
                    $shop_group_id = Shop::getGroupFromShop($shop_id);
                    $conf .= (string)($key > 1 ? ',' : '').Configuration::get('MOD_TMVERTICALMENU_ITEMS', null, $shop_group_id, $shop_id);
                }
            } else {
                $shop_id = (int)$shops[0];
                $shop_group_id = Shop::getGroupFromShop($shop_id);
                $conf = Configuration::get('MOD_TMVERTICALMENU_ITEMS', null, $shop_group_id, $shop_id);
            }
                ...
                case 'LNK':
                    ...
                    if (count($link)) {
                        if (!isset($link[0]['label']) || ($link[0]['label'] == '')) {
                            ...
                            $link = Tm_VerticalMenuTopLinks::get($link[0]['id_tmverticalmenu'], (int)$default_language, (int)Shop::getContextShopID());
                        }
                        $html .= '<option selected="selected" value="LNK'.(int)$link[0]['id_tmverticalmenu'].'">'.Tools::safeOutput($link[0]['label']).'</option>';
                    }
                    break;
		...
		foreach ($menu_items as $item) {
            ...
            switch (Tools::substr($item, 0, Tools::strlen($value[1]))) {
                ...
                case 'SHOP':
                    ...
                    break;
                case 'LNK':
                    $link = Tm_VerticalMenuTopLinks::get($id, $id_lang, $id_shop);
                    if (!empty($link)) {
                        if (!isset($link[0]['label']) || ($link[0]['label'] == '')) {
                            $default_language = Configuration::get('PS_LANG_DEFAULT');
                            $link = Tm_VerticalMenuTopLinks::get($link[0]['id_tmverticalmenu'], $default_language, (int)Shop::getContextShopID());
                        }
                        $root_node['children'][] = $this->makeNode([
                            'type' => 'link',
    ...
    protected function generateCategoriesMenu($categories, $is_children = 0)
    {
        ...
        foreach ($categories as $key => $category) {
            ...
            $node['url'] = $link;
            $node['type'] = 'category';
            $node['page_identifier'] = 'tmcategory-' . $category['id_category'];
    ...
    {
        return _PS_CACHE_DIR_ . DIRECTORY_SEPARATOR . 'tm_verticalmenu';
    }
    ...
    {
        $tmverticalmenu = Db::getInstance()->executeS('
			SELECT *
			FROM '._DB_PREFIX_.'tmverticalmenu
			WHERE id_shop = '.(int)$params['old_id_shop']
            );

        foreach ($tmverticalmenu as $id => $link) {
            Db::getInstance()->execute('
				INSERT IGNORE INTO '._DB_PREFIX_.'tmverticalmenu (id_tmverticalmenu, id_shop, new_window)
				VALUES (null, '.(int)$params['new_id_shop'].', '.(int)$link['new_window'].')');

            $tmverticalmenu[$id]['new_id_tmverticalmenu'] = Db::getInstance()->Insert_ID();
        }

        foreach ($tmverticalmenu as $id => $link) {
            $lang = Db::getInstance()->executeS('
					SELECT id_lang, '.(int)$params['new_id_shop'].', label, link
					FROM '._DB_PREFIX_.'tmverticalmenu_lang
					WHERE id_tmverticalmenu = '.(int)$link['id_tmverticalmenu'].' AND id_shop = '.(int)$params['old_id_shop']);

            foreach ($lang as $l) {
                Db::getInstance()->execute('
					INSERT IGNORE INTO '._DB_PREFIX_.'tmverticalmenu_lang (id_tmverticalmenu, id_lang, id_shop, label, link)
					VALUES ('.(int)$link['new_id_tmverticalmenu'].', '.(int)$l['id_lang'].', '.(int)$params['new_id_shop'].', '.(int)$l['label'].', '.(int)$l['link'].' )');
            }
        }
    }
    ...
    public function renderForm()
    {
        ...
        $helper->tpl_vars = array(
			...
			//'MOD_TMVERTICALMENU_COLS' => $this->getNumberColsMenu(),
        );
    ...
    public function renderAddForm()
    {
        $fields_form = array(
            'form' => array(
                'legend' => array(
                    'title' => (Tools::getIsset('updatetmverticalmenu') && !Tools::getValue('updatetmverticalmenu')) ?
                ...
                'submit' => array(
                    'name' => 'submitTmverticalmenuLinks',
                    'title' => $this->l('Add')
                )
            ),
        );
        ...
        if (Tools::getIsset('updatetmverticalmenu') && !Tools::getValue('updatetmverticalmenu')) {
            $fields_form['form']['submit'] = array(
                'name' => 'updatetmverticalmenu',
                'title' => $this->l('Update')
            );
        }

        if (Tools::isSubmit('updatetmverticalmenu')) {
            $fields_form['form']['input'][] = array('type' => 'hidden', 'name' => 'updatelink');
            $fields_form['form']['input'][] = array('type' => 'hidden', 'name' => 'id_tmverticalmenu');
    ...
    public function renderChoicesSelect()
    {
        ...
        $links = Tm_VerticalMenuTopLinks::gets($this->context->language->id, null, (int)Shop::getContextShopID());
        foreach ($links as $link) {
            if ($link['label'] == '') {
                $default_language = Configuration::get('PS_LANG_DEFAULT');
                $link = Tm_VerticalMenuTopLinks::get($link['id_tmverticalmenu'], $default_language, (int)Shop::getContextShopID());
                if (!in_array('LNK'.(int)$link[0]['id_tmverticalmenu'], $items)) {
                    $html .= '<option value="LNK'.(int)$link[0]['id_tmverticalmenu'].'">'.$spacer.Tools::safeOutput($link[0]['label']).'</option>';
                }
            } elseif (!in_array('LNK'.(int)$link['id_tmverticalmenu'], $items)) {
                $html .= '<option value="LNK'.(int)$link['id_tmverticalmenu'].'">'.$spacer.Tools::safeOutput($link['label']).'</option>';
            }
        }
        $html .= '</optgroup>';
        $html .= '</select>';
    ...
    public function getAddLinkFieldsValues()
    {
        ...
        if (Tools::isSubmit('updatetmverticalmenu')) {
            $link = Tm_VerticalMenuTopLinks::getLinkLang(Tools::getValue('id_tmverticalmenu'), (int)Shop::getContextShopID());
        ...
        $fields_values = array(
            ...
            'id_tmverticalmenu' => Tools::getValue('id_tmverticalmenu'),
        );
    ...
    public function renderList()
    {
        ...
        foreach ($shops as $shop_id) {
            $links = array_merge($links, Tm_VerticalMenuTopLinks::gets((int)$this->context->language->id, null, (int)$shop_id));
        }

        $fields_list = array(
            'id_tmverticalmenu' => array(
        ...
        $helper->identifier = 'id_tmverticalmenu';
        $helper->table = 'tmverticalmenu';
    ...
    {
        $this->context->controller->registerStylesheet('modules-tmverticalmenu', 'modules/'.$this->name.'/views/css/tm_verticalmenu.css', ['media' => 'all', 'priority' => 150]);
    }
    ...
    public function renderWidget($hookName, array $configuration)
    {
        ...
        return $this->fetch('module:tm_verticalmenu/views/templates/hook/tm_verticalmenu.tpl');
    }
	
	private function getNumberColsMenuItems()
	{
		$items_cols = Tools::getValue('MOD_TMVERTICALMENU_COLS');
		...
		if (is_array($items_cols) && count($items_cols)) {
			...
			if (count($shops) > 1) {
				foreach ($shops as $key => $shop_id) {
					$shop_group_id = Shop::getGroupFromShop($shop_id);
					$conf .= ($key > 1 ? ',' : '').((string)Configuration::get('MOD_TMVERTICALMENU_COLS', null, $shop_group_id, $shop_id));
				}
			} else {
				$shop_id = (int)$shops[0];
				$shop_group_id = Shop::getGroupFromShop($shop_id);
				$conf = Configuration::get('MOD_TMVERTICALMENU_COLS', null, $shop_group_id, $shop_id);
			}
