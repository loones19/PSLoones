...
class Tm_Nav1cmsblock extends Module implements WidgetInterface
{
    ...
    public function __construct()
    {
        ...
        Shop::addTableAssociation('tmnav1cmsblockinfo', array('type' => 'shop'));


        $this->displayName = $this->trans('TM - Nav1 CMS Block', array(), 'Modules.Nav1CmsBlock.Admin');
        $this->description = $this->trans('Adds custom information block in your store.', array(), 'Modules.Nav1CmsBlock.Admin');
    ...
    public function installDB()
    {
        ...
        $return &= Db::getInstance()->execute('
                CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'tmnav1cmsblockinfo_shop` (
                `id_tmnav1cmsblockinfo` INT(10) UNSIGNED NOT NULL,
                `id_shop` INT(10) UNSIGNED NOT NULL,
                PRIMARY KEY (`id_tmnav1cmsblockinfo`, `id_shop`)
            ) ENGINE=' . _MYSQL_ENGINE_ . ' DEFAULT CHARSET=utf8 ;'
        );
        ...
        $return &= Db::getInstance()->execute('
                ...
                `text` text NOT NULL,
                PRIMARY KEY (`id_tmnav1cmsblockinfo`, `id_lang`, `id_shop`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=utf8 ;'
        );
    ...
    public function uninstallDB($drop_table = true)
    {
        $ret = true;
        if ($drop_table) {
            $ret &= Db::getInstance()->execute('DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'tmnav1cmsblockinfo`')
                && Db::getInstance()->execute('DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'tmnav1cmsblockinfo_shop`')
                && Db::getInstance()->execute('DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'tmnav1cmsblockinfo_lang`');
        }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('savetm_nav1cmsblock')) {
            if (!Tools::getValue('text_'.(int)Configuration::get('PS_LANG_DEFAULT'), false)) {
                ...
                $update = $this->processSaveNav1CmsBlock();
   ...
   public function processSaveNav1CmsBlock()
    {
    ...
    protected function renderForm()
    {
        ...
        $fields_form = array(
            'tinymce' => true,
            'legend' => array(
                'title' => $this->trans('CMS block', array(), 'Modules.Nav1CmsBlock.Admin'),
            ),
            'input' => array(
                ...
                'content' => array(
                    'type' => 'textarea',
                    'label' => $this->trans('Text block', array(), 'Modules.Nav1CmsBlock.Admin'),
                    'lang' => true,
                    'name' => 'text',
                    'cols' => 40,
                    'rows' => 10,
    ...
    public function getFormValues()
    {
        ...
        $idInfo = Nav1CmsBlock::getNav1CmsBlockIdByShop($idShop);

        Shop::setContext(Shop::CONTEXT_SHOP, $idShop);
        $tmnav1cmsblockinfo = new Nav1CmsBlock((int)$idInfo);

        $fields_value['text'] = $tmnav1cmsblockinfo->text;
        $fields_value['id_tmnav1cmsblockinfo'] = $idInfo;
    ...
    {
        $sql = 'SELECT * FROM `'._DB_PREFIX_.'tmnav1cmsblockinfo_lang`
    ...
    public function installFixtures()
    {
        ...
        foreach ($tabTexts as $tab) {
            $tmnav1cmsblockinfo = new Nav1CmsBlock();
            foreach ($languages as $lang) {
                $text[$lang['id_lang']] = $tab['text'];
            }
            $tmnav1cmsblockinfo->text = $text;
            $return &= $tmnav1cmsblockinfo->add();
        }
        ...
        if($return && sizeof($shopsIds) > 1) {
            foreach ($shopsIds as $idShop) {
                Shop::setContext(Shop::CONTEXT_SHOP,$idShop);
                $tmnav1cmsblockinfo->text = $text;
                $return &= $tmnav1cmsblockinfo->save();
            }
        }
    ...
    {
        if ($tmnav1cmsblockinfoId = Nav1CmsBlock::getNav1CmsBlockIdByShop($params['old_id_shop'])) {
            Shop::setContext(Shop::CONTEXT_SHOP, $params['old_id_shop']);
            $oldInfo = new Nav1CmsBlock($infoId);

            Shop::setContext(Shop::CONTEXT_SHOP, $params['new_id_shop']);
            $newInfo = new Nav1CmsBlock($infoId, null, $params['new_id_shop']);
