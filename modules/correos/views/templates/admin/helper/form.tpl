...
{if $option.type eq 'select'}
    <div class="">
        <select autocomplete="" class="form-control" id="{$option.name|escape:'htmlall':'UTF-8'}"
...
{elseif $option.type eq 'hidden'}
    <input autocorrect="off" type="hidden" {if isset($option.name)}name="{$option.name|escape:'htmlall':'UTF-8'}"{/if} {if isset($option.id)}id="{$option.id|escape:'htmlall':'UTF-8'}"{/if}
...
{elseif $option.type eq 'textbox' and isset($option.color) and $option.color}
    <div class="">
        <div class="input-group color-picker">
            <input autocomplete="" type="text" class="form-control" name="{$option.name|escape:'htmlall':'UTF-8'}"
...
{elseif $option.type eq 'textbox'}
        ...
        {else}
            <input autocomplete="" type="text" class="form-control"
...
{elseif $option.type eq 'textarea'}
    <div class="">
        ...
        {else}
            <textarea autocomplete="" class="form-control" id="{$option.name|escape:'htmlall':'UTF-8'}"
                {if isset($option.placeholder)}{$option.placeholder|escape:'htmlall':'UTF-8'}{/if}
...
{elseif $option.type eq 'submit'}
      <button type="submit" class="btn {if isset($option.class)}{$option.class|escape:'htmlall':'UTF-8'}{/if}" id="{$option.id|escape:'htmlall':'UTF-8'}" name="{$option.id|escape:'htmlall':'UTF-8'}">{$option.value|escape:'quotes':'UTF-8'}</button>
      {/if}