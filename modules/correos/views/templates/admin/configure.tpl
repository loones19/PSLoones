      ...
      {if isset($paramsBack.HELPER_FORM.forms) and is_array($paramsBack.HELPER_FORM.forms) and count($paramsBack.HELPER_FORM.forms)}
         {foreach from=$paramsBack.HELPER_FORM.forms key='key' item='form' name='forms'}
            ...
            <div class="tab-pane fade panel {if (isset($CURRENT_FORM) && $CURRENT_FORM eq $form.tab) || (not isset($CURRENT_FORM) && $smarty.foreach.forms.first)}active in{/if}" id="tab-{$form.tab|escape:'htmlall':'UTF-8'}"> 
               ...
               <form autocomplete="on" method="post" class="form clearfix {if isset($form.class)}{$form.class|escape:'htmlall':'UTF-8'}{/if}" {if isset($form.id)}id="{$form.id|escape:'htmlall':'UTF-8'}"{/if} {if isset($form.enctype)}enctype="{$form.enctype|escape:'htmlall':'UTF-8'}"{/if}>
               ...
               {foreach from=$form.options item='option'}
                        ...
                        <div class="form-group">
                                 {if isset($option.label)}
                                          ...
                                          <label class="control-label {if isset($option.label_class)}{$option.label_class|escape:'htmlall':'UTF-8'}{/if}">
...
{/if}
   ...
   <div class="tab-pane fade panel {if (isset($CURRENT_FORM) && $CURRENT_FORM eq 'search_rma')}active in{/if} form-horizontal" id="tab-search_rma"> 
   {include file=$paramsBack.CORREOS_TPL|cat:'views/templates/admin/helper/search_rma.tpl' 
     correos_dir=$paramsBack.CORREOS_DIR
     tpl_dir=$paramsBack.CORREOS_TPL
     returns=$paramsBack.RETURNS
     correos_token=$paramsBack.CORREOS_TOKEN
      }
   </div>
   ...
   <div class="tab-pane fade panel {if (isset($CURRENT_FORM) && $CURRENT_FORM eq 'request_collection')}active in{/if}" id="tab-request_collection"> 
   {include file=$paramsBack.CORREOS_TPL|cat:'views/templates/admin/helper/request_collection.tpl' 
      correos_config=$CORREOS_CONFIG
      sender_form=$paramsBack.HELPER_FORM.forms.sender
      }
   </div>
