	...
	if (CorreosConfig.carrierCityPaq.map(Number).indexOf(CorreosConfig.selectedCarrier) >= 0) {	
      ...
      if(jQuery(".supercheckout_shipping_option").length) 
       ...
       // setTimeout(function(){ 
          ...
          var element = $('#correospaq').clone();
          $('#correospaq').remove();
          $('input[type=radio].delivery_option_radio:checked').closest( ".delivery_option" ).append(element);
          $('#correospaq').fadeIn();
          
        //}, 1000);
        //jQuery('#correospaq').detach().appendTo($('input[type=radio].delivery_option_radio:checked').closest( ".delivery_option" ))
      } else
